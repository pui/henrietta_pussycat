# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{henrietta_pussycat}
  s.version = "0.1.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 1.2") if s.respond_to? :required_rubygems_version=
  s.authors = ["Erica Kwan"]
  s.date = %q{2009-03-15}
  s.description = %q{Replaces words with 'meow' in strings in different ways}
  s.email = %q{erica.kwan@gmail.com}
  s.files = ["Rakefile", "readme.rdoc", "Manifest", "henrietta_pussycat.gemspec"]
  s.has_rdoc = true
  s.homepage = %q{}
  s.rdoc_options = ["--line-numbers", "--inline-source", "--title", "Henrietta_pussycat", "--main", "readme.rdoc"]
  s.require_paths = ["lib"]
  s.rubyforge_project = %q{henrietta_pussycat}
  s.rubygems_version = %q{1.3.1}
  s.summary = %q{Replaces words with 'meow' in strings in different ways}

  if s.respond_to? :specification_version then
    current_version = Gem::Specification::CURRENT_SPECIFICATION_VERSION
    s.specification_version = 2

    if Gem::Version.new(Gem::RubyGemsVersion) >= Gem::Version.new('1.2.0') then
    else
    end
  else
  end
end
